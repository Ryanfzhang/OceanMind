---
skill_id: ocean_derived_eof_analysis
description: Computes a derived diagnostic field and performs EOF analysis on that diagnostic.
input_intent: Derived diagnostic request such as vorticity, speed, buoyancy frequency, shear, strain, or Rossby number with region, time, and depth selection.
output_intent: EOF modes, principal components, and variance fractions for a derived diagnostic field.
avoid_when:
- Use ocean_eof_analysis for EOF of a raw variable.
- Use transport_analysis for streamfunction or transport EOF requests.
composes_with:
- ocean_masking_workflow
---
# Ocean Derived EOF Analysis

## Purpose

This skill computes a derived diagnostic field and then performs EOF analysis on either the diagnostic field itself, a feature-depth field, or a layer-mean diagnostic field. Do not use it for volume transport streamfunction, transport st...
...
n_modes = 3  # <-- MODIFY: requested EOF mode count.
preprocessing = 'anomaly'  # <-- MODIFY: preprocessing for EOF, such as anomaly or demean.
weight_by_latitude = True  # <-- MODIFY: usually True for map-like EOFs.
