---
skill_id: ocean_derived_timeseries_analysis
description: Computes a derived diagnostic field and analyzes its time series.
input_intent: Derived diagnostic source variables with region or point selection, time range, depth or layer selection, and optional vertical feature rules.
output_intent: Time series or trend-ready series of a derived diagnostic.
avoid_when:
- Use ocean_timeseries for raw-variable time series.
- Use derived_hovmoller for time-depth diagrams and derived_spatial_field for maps.
composes_with:
- ocean_masking_workflow
---
# Ocean Derived Time Series Analysis

## Purpose

This skill computes a derived diagnostic field first, then analyzes its time series. It supports the same vertical-feature workflow as oceantimeseries, but applies layer averaging to derivedfield. Use this skill only when the requested t...
...
depth_aggregation = 'mean'  # <-- MODIFY: how to reduce a depth range for the regional series.
...
### Stage 4: Regional time series

```python
derived_timeseries = extract_regional_mean(
