---
skill_id: ocean_derived_field_analysis
description: Computes a derived diagnostic field as the final product.
input_intent: Source variables needed for a derived diagnostic such as u/v for vorticity or speed, or temp/salt for density-related diagnostics.
output_intent: Derived diagnostic data container without reducing to a map, time series, profile, histogram, EOF, or Hovmoller.
avoid_when:
- Use derived_spatial_field, derived_timeseries, derived_profile, derived_histogram, derived_eof, or derived_hovmoller when a specific output shape is requested.
composes_with:
- ocean_masking_workflow
---
# Ocean Derived Field Analysis

## Purpose

This skill computes derived or diagnostic ocean fields from loaded variables. Use this skill only when the requested output is itself a derived diagnostic field. Do not use it for raw temperature, salinity, oxygen, chlorophyll, or SST ma...
...
### Stage 3: Assemble and derive diagnostic field
