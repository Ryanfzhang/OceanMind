...
运行额度在每次模型调用和工具执行前检查。总轮数、总时长以及单次代码运行时长各有上限。重复执行同一失败动作、没有新结果时需要换策略；额度耗尽返回未完成状态和现有产物。
...
| 运行额度 | 已用轮数、截止时间、连续失败记录 |
| 结束状态 | `completed`、`needs_input`、`incomplete` 或 `failed` |
...
- 前端阶段卡优先展示成功/失败数量、结果集合摘要和少量明确标注范围的预览。地图当前显示哪一时刻、哪一层必须明确，不能把最后一次检测当成全部结果。
- 沿用现有结果卡、`ownerStepId` 和 `workspace_data` 约定展示支持的结果。大数组留在后端；完整索引可读取/导出，单份结果按 ID 获取，不在每次进度更新中重复发送所有历史结果。
- 先用两层检测验证现有页面能分别选择与查看两份结果，再验证批量结果的摘要与读取。现有前端尚未证明具备分页查看全部产物的能力；如果现有入口无法访问完整索引，需把最小前端适配单独列为待检查的小步，不能声称只靠后端保存就已经实现了全部结果浏览。
...
- **完成：** agent 已核对用户要求的交付物；检测/计算类任务按输入范围、方法与结果验收，综合解释类任务还需核对结论依据。`finalize` 只检查结果引用、文件和成功执行记录，不强制每类任务都有证据报告。用户要求代码时附上实际运行版本。
- **需要输入：** 返回继续任务所必需的具体问题；保存会话消息与产物引用，用户补充后继续。无需让用户回答 agent 可自行从数据读取的信息。
- **未完成或失败：** 明确哪些步骤成功、哪些受阻及原因；保留可复查的代码和结果。

程序可以检查文件、执行状态和引用，科学合理性由 agent 结合数值核查与图像观察判断，并通过样例验收；不能把文件存在当成科学结论正确。

前端展示实际发生的阶段、进度与结果摘要；后台保留完整调用记录和逐切片结果。重跑产生新的尝试及阶段 ID，并标明修订关系，不覆盖旧失败尝试。保留简短行动说明和可复查结果，不展示模型内部推理。
...
- [x] **B8.2 重复失败限制。** 记录动作名称、参数摘要和错误，限制同一动作原样重试。验收：连续重复失败会终止或改变动作，不能无限循环。
...
E 的实现和验收（2026-09-25）：代码按 run 保存不可变版本，修订保留差异；独立进程执行保存的版本，逐次记录 `tools.*` 和 `publish`，实时发送阶段事件，保留跨尝试结果引用。执行观察只返回有限阶段、产物与错误摘要；完整结果可通过 `list_results` 分页查找，日志保存后按需读取。Mac 宿主使用 `sandbox-exec` 的 deny-default 策略；每次运行前实测授权数据可读、任务产物可写，且无关文件、网络、代码/日志改写及子进程派生被拒绝。其他宿主或隔离验证失败时拒绝执行。脚本仍可写其任务目录内的 records 与 artifact index，因此父进程校验路径、归属、代码版本与结果文件存在性；这不是对脚本自身记录内容的独立签名认证。
