    ...
    *,
    ...
    if analysis_session is not None:
        ...
        analysis_prompt = (
            ...
            "A saved two-dimensional field with lat/lon coordinates is shown on the "
            "main interactive map, including a selected raw field. If the user asks "
            "to show a spatial field, publish that field as a DataArray with its "
            "coordinates intact (or use its existing saved result in this turn). "
            "For a field saved in an earlier turn, load_result by artifact ID and "
            "publish it in the current turn with that ID in inputs. Prefer this "
            "interactive map to a standalone PNG for ordinary spatial maps. "
            "Publish a PNG only when the user asks for a static figure or the "
            "visualization cannot be represented as a map layer. "
