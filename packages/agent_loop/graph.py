    ...
    *,
    ...
    if analysis_session is not None:
        ...
        analysis_prompt = (
            ...
            "For multi-year time-depth data with a meaningful seasonal cycle, keep "
            "the original result and compute and publish a separate climatology from "
            "its full values, not the chart preview. Use the same "
            "time/spatial_coord/values structure and label its aggregation in metadata. "
            "For daily data, align the seasonal cycle by calendar day across years, "
            "smoothing it if useful. "
            "Without a meaningful seasonal cycle, show only the original result. "
