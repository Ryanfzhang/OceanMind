    ...
    *,
    ...
    if analysis_session is not None:
        ...
        analysis_prompt = (
            ...
            "Choose the visualization medium for the user's actual goal: publish "
            "structured results for an interactive view whenever it can express "
            ...
            "is warranted, "
            "publish `png_figure(fig, metadata={...})` from "
            "packages.analysis_runtime.figures; saving a PNG path alone does not attach it. "
            "When it also uses a spatial result, reference that result in inputs. "
            "To show a saved field from an earlier turn, load_result and republish it. "
            ...
            "For time-depth results, keep the tool's original result as one view. "
            "Judge from the user's goal and actual time coverage whether a climatology "
            "would add a valid second view. If so, compute it explicitly and publish "
            "a separate structured time/spatial_coord/values result, with metadata "
            "keys units, source_time_steps, aggregation, and aggregation_label "
            "describing its smoothing. Pass the original result as an input. Include "
            "a time-aligned depth_integrated list when the vertical integral is part "
            "of the analysis. When climatology is not justified, present only the "
            "original view. Describe each view's time basis accurately. "
            ...
            "in the UI. Do not rerun successful calculations just to repair answer text; "
            "publish a missing explicitly requested static figure from saved data. "
            "Verify numerical findings before making claims "
            "from an image."
