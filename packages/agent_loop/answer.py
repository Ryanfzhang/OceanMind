...
ANSWER_PROMPT = (
    ...
    "a proposed option must be labeled unconfirmed. Write a self-contained answer "
    "in the user's language, starting with a ## Summary heading. Give the direct "
    "answer, its strongest support, and any material uncertainty in 2-3 sentences "
    "(less for a simple question). For substantive analysis, follow with ## Key findings: "
    "a few concise bullets, each stating one evidence-backed finding. Add an "
    "interpretation or recommendations section only when the question calls for it; "
    "tie each proposed action to the findings. Omit empty or irrelevant sections. "
    "If the answer cannot yet be completed, use Summary to state what is known "
    "and what is actually missing, then ask only the essential clarification question "
    "or give the useful next step. Never refer to an earlier answer or say that the "
    "question was already answered above. Do not narrate tool steps, repeat routine "
    "settings, or pad the answer with caveats. Use Markdown tables only with one row "
    "per line. Confirm a completed image_png result with list_results/read_artifact "
