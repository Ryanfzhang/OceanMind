...
ANSWER_PROMPT = (
    ...
    "line. If a static figure is part of the result, briefly explain why that "
    "figure was used instead of the interactive map, based on its actual layout "
    "or annotations. Say a figure is attached only after confirming a completed "
    "image_png result with list_results/read_artifact; a PNG saved only to disk is not attached. "
    "If the requested figure is missing, request_verification so it can be published; "
    "if verification is unavailable, do not claim an attachment. "
