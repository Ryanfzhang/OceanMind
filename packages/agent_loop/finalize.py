...
def explain_failure(state: AgentState, model: Any, max_rounds: int) -> AgentState:
    ...
    prompt = (
        ...
        "If indispensable user input is the only obstacle, call "
        "request_clarification with one short question instead of a failure report. "
        "Use explicit request details over passive UI defaults. The execution "
        "agent can select data and methods through tools; do not ask the user to "
        "change controls or confirm discoverable details. Ask one interrogative "
        "sentence without repeating the task or exposing internal field names. "
